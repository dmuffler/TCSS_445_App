...
require('db.php');
$sql_statement;
$create_statement = "INSERT INTO Rating (instructor_email, student_email, title, score, hotness, comment, date)
                     VALUES (?, ?, ?, ?, ?, ?, NOW())";
$read_statement = 'SELECT * FROM Rating WHERE instructor_email = ? AND student_email = ?';
$read_all_statement = 'SELECT * FROM Rating WHERE instructor_email = ?';
$update_statement = 'UPDATE Rating SET title = ?, score = ?, hotness = ?, comment = ?, date = NOW() WHERE instructor_email = ? AND student_email = ?';
...
if ($verb == 'CREATE') {
  ...
  $arguments = array($instructor_email, $student_email, $title, $score, $hotness, $comment);
  ...
  $hotness = $_GET['hotness'];
  $comment = $_GET['comment'];
  $arguments = array($title, $score, $hotness, $comment, $instructor_email, $student_email);
} else if ($verb == 'DELETE') {
